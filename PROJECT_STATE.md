...
| 项 | 值 |
| --- | --- |
| 当前开发阶段 | Phase 6U（真实环境测试迭代）已完成；冻结与稳定性检查通过（2026-08-10）；**Phase 7 实施中（M0 环境脚手架、M1 后端侧车、M2 前端适配、M3 设置系统、M4 workspace 适配与迁移、M5 桌面集成、M6 构建安装包均已完成，进入 M7 回归发布；决策点 D1-D7 已确认并冻结）** |
| 当前版本号 | **v0.7.3**（唯一来源 `backend/app/__init__.py`，`frontend/src/version.ts` 与 `frontend/package.json` 同步） |
...
1. Phase 7：Tauri 桌面化（M0 环境脚手架、M1 后端侧车、M2 前端适配、M3 设置系统、M4 workspace 适配与迁移、M5 桌面集成、M6 构建安装包均已完成），总纲见 `docs/phase7-plan.md`（决策点已冻结）；剩余里程碑：M7 回归发布 v1.0.0（回归矩阵全绿 + 版本三同步一致 → 出 v1.0.0 安装包）。
2. 前端主包体积优化（非阻塞）。
