...
import { KE_FIELD_ORDER, KE_VERSION, keJson, keStableId, stripFrontmatter, withFrontmatter } from './ke'
...
describe('T0：编辑→保存→重开→字节比较（覆盖 P0-1/P0-2/P1-1/P1-2 前端面）', () => {
  it('frontmatter 含 title/tags/自定义键 + HTML 注释的文档全链路不丢字节', () => {
    const source = [
      '---',
      'ke_version: 1',
      'title: T0题',
    ...
    // 1) 加载（与 EditorArea 一致：剥离 frontmatter 后仅正文进入 Document Model）
    const stripped = stripFrontmatter(source)
    expect(stripped.version).toBe(1)
    const ed = makeEditor(stripped.content)
    // 2) 编辑：在文档开头输入（模拟用户输入；并入首个标题段落）
    ed.commands.insertContent('编辑新增之文本；')
    // 3) 保存：withFrontmatter（编辑器输出 + 版本头；frontmatter 键保留由
    //    withFrontmatter 的合并语义（P0-1 用例）与后端 merge_frontmatter（pytest T0）双保险）
    const savedMd = withFrontmatter(ed.getMarkdown(), KE_VERSION)
    expect(savedMd.match(/^ke_version: \d+$/m)?.length).toBe(1)
    // 4) 重开：剥离 frontmatter → 新编辑器 → 再保存
    const reopenedBody = stripFrontmatter(savedMd).content
    const reopened = withFrontmatter(makeEditor(reopenedBody).getMarkdown(), KE_VERSION)
    // T0 断言：重开后内容与首次保存完全一致（幂等、无漂移；仅容忍行尾空行归一化）
    const norm = (x: string) => x.replace(/[\r\n]+$/, '')
    expect(norm(reopened)).toBe(norm(savedMd))
    // 内容标记（P1-2 HTML 注释/块）与编辑内容均保留
    expect(reopened).toContain('<!-- 普通注释 -->')
    expect(reopened).toContain('<div class="x">内容</div>')
    expect(reopened).toContain('编辑新增之文本；')
    // ke_version 唯一且正文不含 frontmatter 残留
    expect(reopened.match(/ke_version/g)?.length).toBe(1)
    expect(reopened).toContain('标题')
    ed.destroy()
  })
})

describe('P0-4 + P1-1：加载不触发更新 / 清空 undo 历史', () => {
  it('加载内容不触发 update（计数为 0），用户输入后 can().undo() 恢复 true', () => {
    let updates = 0
    const ed = makeEditor('')
    ed.on('update', () => {
