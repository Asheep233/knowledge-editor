...
> 最后更新：2026-09-15（**v1.1.8 发布后：品牌 logo 替换 + 附件区改造**）

## 2026-09-15（发布后 · 品牌 logo 替换 + 附件区改造）
...
**⚠ 已知风险（待主理人裁决，未处置）**：`tauri icon` 图标集取自**「浅色主题」icon 素材**
（深色字形，不透明区平均 RGB (3,6,12)，仅 13% 像素不透明）；旧图标是**满底方形**
（bbox 100%、96.1% 不透明、平均 RGB (229,230,234)）。
→ 透明深字标在**深色任务栏**下对比度低（浅色任务栏实测可见）。若需两种主题都清晰，
应改用「品牌色/浅色底板 + 字形」的图标源重新 `tauri icon`（应用内透明标不受影响，可保留）。
