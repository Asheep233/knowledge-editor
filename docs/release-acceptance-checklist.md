...
| 项 | 值 |
|---|---|
| 验收版本 | **v1.2.0-pre.2**（预发布；Latest 仍为 v1.1.9）|
| 验收包 | `AstraNota_1.2.0-pre.2_x64-setup.exe`（sha256 `1fb969fcf7654fef3464f2fef84b932ae882d5429a784427c38eef79b1eae9c3`，50,792,339 B）|
| 环境 | 主理人机器（Windows + WebView2）· 覆盖安装/直接运行 release 产物 |
| 日期 / 验收人 | 2026-09-20 / Lead（CDP 驱动自动化）|
| 结论 | 通过（本轮为**发行前 GUI 验收**，非干净机首装演练）|

**v1.2.0-pre.2 已跑项（实际证据）**
| 项 | 结果 |
|---|---|
| 版本一致性 | 界面横幅 `v1.2.0-pre.2` ↔ 后端 `v1.2.0-pre.2`，**无版本不一致提示** |
| 标签栏 | 打开文档即入标签（`[role=tablist]` + `[role=tab]`） |
| 设置页左栏高亮 | 四态全对：顶部→常规 / 滚到各组→对应项 / 回顶回落 / 点击跟随 |
| **源码模式端到端** | 切源码 → textarea = 正文原文且 frontmatter 隐藏、ProseMirror 卸载 → 追加 2 行 → 自动保存 → **磁盘与原文件逐字节差异仅那 2 行**（frontmatter/正文/`ke-*` 注释/换行风格全保留）→ 切回正文恢复 ProseMirror |
| 门禁 | pytest 630+2 · tsc 0 · vitest **56 files / 1095 passed + 1 skipped** · cargo 20 |
| 独立对抗验证 | 保真 47 例 + 源码模式 49 例 + ①②+Tab 42/30 例（详见各 `docs/verification-*.md`） |
| 附件区 | 左栏「附件」默认收起，表头保留「附件 N + 孤儿附件 N」徽章（pre.1 起） |
| 快捷键设置 | 设置页「快捷键」组：录制/解绑/恢复默认 + 保留键提示（pre.1 起） |

**仍待做（正式版 `1.2.0` 前）**：干净 Windows 首装演练（清空 `%APPDATA%\KnowledgeEditor` 模拟首装）+ 附件/导出四件产物字节校验在新版本上的复跑（上次执行于 v1.1.3）。
> 验收后补充（2026-09-06）：主理人自 GitHub 安装 v1.1.3 成品包，在 `D:\Download` 手动导出四件产物，
> 字节校验全部通过——① KE 导出 vs 磁盘源文档 diff=0；② 普通导出 ke-/ke_version 残留=0（信息块正确降级为引用块）；
> ③ zip 内嵌 md 与① 同 sha256；④ 二次导出与① 同 sha256（无静默失败）。
...
- [ ] 三种导出零改动；多标签 TabBar 仍为拍板延后；**回收站已于 2026-09-15 立项（不再是延后项）**
...
1. **Windows 真机 GUI 验证**：CDP 自动化已覆盖 A1-A3/A5-A7（2026-09-06 执行）；A4 真实字节落盘由主理人手动导出闭合（同日晚）。
