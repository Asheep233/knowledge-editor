...
当前版本 **v1.1.6-pre.1**（修复预发布 · 正式版见 Releases）· Windows 安装包见 [GitHub Releases](https://github.com/Asheep233/knowledge-editor/releases)
...
> 随本版一并修复：「新建文件夹/新建文档」不可用（WebView2 下 window.prompt 输入不返回 → 自绘输入弹窗）、
> 空文件夹树中不可见、树展示扁平化、同名文件夹显式提示、输入框内容残留。
> 修复版本（不含品牌）见 v1.1.4-pre.1。

本项目开发过程包含 AI Agent 协作，透明性声明见 [docs/agent-collaboration.md](docs/agent-collaboration.md)。

## 安装（Windows 桌面版）

1. 从 [Releases](https://github.com/Asheep233/knowledge-editor/releases) 下载 `AstraNota_1.1.6-pre.1_x64-setup.exe`（当前正式安装包）。
2. 双击安装：安装到 `%LOCALAPPDATA%\AstraNota`，开始菜单创建快捷方式。
3. 首次启动选择「使用已有工作区」或「创建新工作区」，即可开始写作。

程序与数据分离：应用设置与数据位于 `%APPDATA%\KnowledgeEditor`（品牌改版未改应用标识，路径不变；旧版数据自动衔接），文档工作区由用户选择（默认如 `文档\KE Workspace`），卸载软件不删除数据。安装包未签名，首次运行若出现「无法验证发布者」提示属正常现象（Windows 按路径记忆确认），确认即可。
...
- 品牌改版（v1.1.4）：更名 AstraNota；应用图标全套重生成（白底轨道-A 形标，22% 圆角）；设置页右列横版品牌横幅（浅色版 + 深色透明抠图版按主题自动切换）；任务栏图标 256px 源（根治发糊）。
