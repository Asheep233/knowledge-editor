...
**v1.2.2 补丁版已发布（Latest）**（`1.2.0-pre.1`、`1.2.0-pre.2`，Latest 仍是 `v1.1.9`）；
...
| 当前版本 | `1.2.2`（**补丁版已发布，Latest**）（发布前全面审查 B1-B5/M1-M8/U4 修复 + 对抗验证 R-1…R-4b + 真机发现的退出挂死与 Ctrl+Q 加速键问题；`node scripts/bump-version.mjs <ver>` 维护） |
