...
### 拍板延后项复核（不做，仅维护状态）
- 多标签 TabBar、音频上传白名单、字号设置空壳 —— 拍板延后；
  不消耗迭代资源，除非主理人明确立项。
- **回收站：✅ 已于 2026-09-15 由主理人明确立项**（推翻原「延后」拍板）。
  可行性评估见 `docs/design-file-management-feasibility.md`（含落点实测「根级 `Trash/`
  零扫描器改动」、软删 314 用例零回归实测、四方案对比、MVP ≈5.5–7 人天、待拍板决策清单）。
  原占位 disabled 状态由本次立项解除。
