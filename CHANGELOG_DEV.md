...
| 提交 | 内容 | 验证 |
|---|---|---|
| `d609f08` | **复制公式后编辑第二行却改到第一行**（用户实测，静默写错节点）。根因：① 复制粘贴保留公式 `id` → 两行同 id；② `locateMathById` 原为「id 优先 + 取**第一个**命中」→ 保存命中的永远是第一行。修法两层：**A** 新增 `MathIdUniqueness`（`appendTransaction` 去重，仅 math/mathBlock，单事务可撤销）+ **B** `locateMathById` 改 **pos 优先 + 最近邻回退**（点哪改哪，已存在的重复 id 文档也能正确编辑） | 新增 8 例（含修前会红的复现用例）+ 全量 vitest **62 files / 1177 passed + 1 skipped**；真机已复现「复制→两份相同公式」，最终改第二行只动第二行待主理人复验 |
...
| 提交 | 内容 | 验证 |
|---|---|---|
| `c195298` | **「点 ⋮ 没反应」**：公式互转菜单用 `absolute right-0 top-full`，但 `.ke-math` 没有定位上下文 → 菜单锚到 `.ProseMirror{position:relative}`，实测被放到 **(1090,2078) 视口外**。修法 `.ke-math{position:relative}`；复测 `offsetParent=ke-math--inline`、菜单 (452,507) 在视口内 | 真实鼠标事件 + rect 断言（脚本 `element.click()` 会漏掉此类问题，已写进交接文档 12c） |
| `500d506` | **新功能：公式互转（行内 ⇄ 行间）**（主理人口径：不自动开编辑器、光标落新块之后、多行 LaTeX 拒绝、入口 = 笔按钮旁竖排 ⋮）。实现：`editor/math/convert.ts` 两个**单事务**命令（进命令前预检避免 tiptap 空事务；只读态 0 事务；多行拒绝时文档逐字节不变）+ NodeView ⋮ 菜单（只发 window 事件，事务在 EditorArea 执行以规避 React #300）+ 转换后**交还键盘焦点**（真机验收发现：⋮ 是 tabIndex=0 的 span，夺焦点后 Ctrl+Z/打字不生效） | 新增 16 例（双向/光标/一次撤销/往返/相邻公式 `$a$$b$` 回归/多行拒绝/只读/边界/接线守卫）；真机：⋮ 菜单可点开 · 行内→行间 2/1→1/2 · **未点编辑器直接 Ctrl+Z 即撤回** ✓ |
