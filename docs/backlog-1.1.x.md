...
| **F-S1-4** | **①** 大文档（>200KB）80ms 延迟载入的过期 timeout 仍会 `setKeContent` 覆盖编辑器（切档过快时）；**②** `flushDraftRecovery`（`EditorArea.tsx:239-244`）用 `articleRef.current` + 编辑器实时内容，**缺 `editorDocIdRef` 守卫**（与 save 路径不同源） | 验证 F-S1-2 时由 verifier 独立构造证实：`[DS6]` C 载入后 80ms 变 `B-BODY`；`[DS7]` 破坏态下**以 C 的名义登记了含 B 正文的恢复点**（C 的 PUT = 0，save 路径无串写） | **数据完整性**：崩溃后若「恢复」，会把 B 的内容写回 C 的路径（跨文档内容污染） | 🔧 修复中（task-19/20）：① 代次令牌/清理过期 timeout；② `flushDraftRecovery` 增加 `editorDocIdRef.current === doc.id` 守卫并与 save 路径同源 |
