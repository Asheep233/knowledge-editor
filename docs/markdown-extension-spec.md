...
| P2 | 幂等性 | 未知 `ke-*` 标记必须原样保留，**任何工具不得自动删除或改写**。例外（v1.2.0）：源码模式（`document-format.md` §6）下**用户明确改写**属于授权行为，保存时必须提示，不视为违反本原则 |
...
| 非 `ke-` 前缀的普通注释 | 一律原样保留 |
| GFM 脚注 `[^label]` / `[^label]: 内容` | **等价解析**为 `footnote` / `footnotes` 节点（v1.1.8，见 `document-format.md` §2.3.1）；保存后输出 ke 方言 |

> 规则 P3 的推论：**任何未知标记都不会导致文档损坏、内容丢失或渲染崩溃**。
> 这是对第三方 Markdown 编辑器友好性的硬性要求（决策点 2）。

## 3. 节点定义
> **v1.1.5 ③ 颜色取值扩展**：`color` 字段支持三类取值——
> 1. 预设标识：`blue / yellow / green / red / purple / cyan / orange / slate`；
> 2. 特殊标识：`accent`（跟随应用强调色）；
> 3. 任意十六进制色号：`#RRGGBB` 或无 `#` 的 `RRGGBB`（数据面直存；渲染时按亮度自动
>    求可读前景色）。旧文档（仅预设标识/无 color）向后兼容，UI 缺省仍为 accent 系淡蓝。

> **v1.1.7（A）内容语义扩展**：`ke-note` 包裹格式的块内内容由行内片段升级为
> **块级 Markdown**（列表/多段落/加粗段落等整体进入信息块）——粘贴或书写多块内容不再溢出
> 信息块外。包裹格式（`<!-- ke-note: json -->` ... `<!-- /ke-note -->`）不变；旧行内内容文档
> 兼容（单段文本解析为段落，往返稳定）。
...
| `source` | 否 | 模块来源相对路径（Phase 5 记录，不参与动态同步）。**源文件被移动/改名/删除后该记录可能悬空，属预期**：v1 采用「插入后复制内容」语义，模块正文在插入时已复制进引用方文档，运行时不解析 `source`（后端零消费者、编辑器内为隐藏占位）。故移动模块文档**不做拦截也不做提示**（主理人 2026-09-15 裁决「方案 C：只登记」，见 `docs/backlog-1.1.x.md` F10 行）；将来若引入动态模块同步，需一并处理悬空 `source` 的迁移或兜底匹配（必填字段 `id` 可作匹配依据） |
...
> **v1.1.8 脚注方言提示**：GFM 脚注文档（`[^1]` / `[^1]: 内容`）被本产品打开并保存后，
> 会输出为 ke 方言（`<!-- ke-footnote -->` + `<!-- ke-footnotes -->` 区域）。此时在 GitHub /
> Typora 上查看该文件，脚注标记会被当作 HTML 注释忽略（与其它 `ke-*` 节点一致）。
> 需要保持外部可读时，用「**导出普通 Markdown**」——`plain-export` 会还原为 GFM 脚注语法。
