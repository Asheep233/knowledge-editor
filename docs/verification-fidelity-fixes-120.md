...
| **D-2 嵌套未知标签** | ✅ **契约口径 PASS（47/47）** —— 契约（= **D-5**，主理人 2026-09-19 裁决「只要求集合保全 + 幂等，不要求嵌套顺序镜像」）三项均满足：**① 标准转换在场**（`<em>x</em>`→`*x*` 等，未回归）**② 内层未知标签逐字节在场**（含属性含 `>`、大小写、自闭合）**③ 幂等**；另有代码/围栏字面、文本不丢。**已知偏差 D-5**：嵌套顺序与原文镜像不同（`<em><span>x</span></em>` → `<span>*x*</span>`），已写入 `document-format.md` §2.6，见 §4 |
...
## 4. D-2 嵌套未知标签（契约口径 PASS；嵌套顺序差异 = 已知偏差 **D-5**）

> **归因：D-5 —— 主理人 2026-09-19 裁决「契约只要求集合保全与幂等，不要求嵌套顺序镜像」**（已写入 `docs/document-format.md` §2.6，提交 `c1fb944`）。
> **契约口径三项**：① **标准转换在场**；② **内层未知标签逐字节在场**（含属性、大小写、自闭合形态）；③ **幂等**。三者在本轮实测中全部满足（D2-1..D2-14 全绿）。
> 参照形态：`<span><em>x</em></span>` → `<span>*x*</span>`（未知在外、标准在内，与本实现输出一致）。
> 裁决理由（引自规范）：要拿 `*<span>x</span>*` 镜像形态，必须把未知标签连内容整体原子化并自行补 mark → `<em>a<span>x</span>b</em>` 这类混合形态会产出 `*a**<span>x</span>**b*` 双星号（**更糟的损坏路径**），故按现状声明为已知偏差。
...
3. **D-2 ✅ 契约口径通过（= D-5 裁决态）**：① 标准转换在场 ② 内层未知标签逐字节在场 ③ 幂等 —— 三项契约全部满足；嵌套顺序差异（未知标签外置）已由主理人裁决为**已知偏差 D-5**（`document-format.md` §2.6，`c1fb944`），报告 §4 保留 6 条严格形态实测表与每次运行的 `D2-x-FORM` 日志作为持续观察手段。**若 final 1.2.0 需恢复严格断言**（或补测 `KeBlockJoin` 在表格单元格/多级引用块的组合），cold-resume 我即可。
